...
import {
  GoogleFileManagerService,
  FileAlertAction,
} from './google-file-manager.service';
...
export class DataService {
      ...
      .subscribe(({ action, gloomFile }) => {
        ...
        if (action === 'load') {
          ...
          if (i >= 0) {
            ...
            console.warn(
              'Warning: data service and file manager out of sync. Correcting... '
            );
            this.gloomFiles = Array.from(
              this.fileManager.currentDocuments.values()
            ).map((val) => new GloomFile(val));
          }
        }
  ...
  listenCharactersFiles(): Observable<CharacterFile[]> {
    ...
    const newVal$ = this.characterFileAlert$.pipe(
      map(() => this.getCharacterFiles())
    );
    return merge(currVal$, newVal$);
  }
  ...
  listenCampaignMinis(): Observable<CampaignMini[]> {
    ...
    const newVal$ = this.campaignFileAlert$.pipe(
      map(() => this.getCampaignMinis())
    );
    return merge(currVal$, newVal$);
  }
  ...
  getCharacterMinis(): CharacterMini[] {
    return this.gloomFiles
      ...
      .map(
        (charFile) =>
          new CharacterMini(charFile.id, charFile.content.Character.name)
      );
  }
  ...
  listenCharacterFileByDocId(
    docId: string
  ): Observable<{ action: FileAlertAction; file: CharacterFile }> {
  ...
  createNewCharacter(name: string, gclass: string): Observable<CharacterFile> {
    ...
    return this.fileManager
      .createAndSaveNewJsonFile(name, { Character: char })
      .pipe(
